...
import { observabilityService } from "@blocks-devops/services/observability.service";
...
vi.mock("@blocks-devops/services/observability.service", () => mockObservabilityServiceFactory());
...
describe("Observability Hooks", () => {
  ...
  describe("useGetSASTData", () => {
    it("should fetch SAST data successfully", async () => {
      ...
      expect(observabilityService.SASTData).toHaveBeenCalledWith(MOCK_BUILD_ID, TEST_PROJECT_KEY);
    });
  });
  ...
  describe("useGetSCALibraryData", () => {
    it("should fetch SCA library data successfully", async () => {
      vi.mocked(observabilityService.SCAData).mockResolvedValue("sca-library-report");
      ...
      expect(observabilityService.SCAData).toHaveBeenCalledWith(MOCK_BUILD_ID, TEST_PROJECT_KEY, "libraries");
    });
  });
  ...
  describe("useSCARedirectLink", () => {
    it("should fetch SCA redirect link successfully", async () => {
      vi.mocked(observabilityService.SCARedirect).mockResolvedValue("sca-redirect-url");
      ...
      expect(observabilityService.SCARedirect).toHaveBeenCalledWith(MOCK_BUILD_ID, TEST_PROJECT_KEY);
    });
  });
  ...
  describe("useGetSCADContainerData", () => {
    it("should fetch SCA container data successfully", async () => {
      vi.mocked(observabilityService.SCAData).mockResolvedValue("sca-container-report");

      const { result } = renderHook(() => useGetSCADContainerData(MOCK_BUILD_ID), {
        wrapper: createWrapper(),
      });

      await waitFor(() => expect(result.current.isSuccess).toBe(true));
      expect(result.current.data).toBe("sca-container-report");
      expect(observabilityService.SCAData).toHaveBeenCalledWith(MOCK_BUILD_ID, TEST_PROJECT_KEY, "container");
    });
  });
  ...
  describe("useSASTRedirectLink", () => {
    it("should fetch SAST redirect link successfully", async () => {
      vi.mocked(observabilityService.SASTRedirect).mockResolvedValue("sast-redirect-url");
      ...
      expect(observabilityService.SASTRedirect).toHaveBeenCalledWith(MOCK_BUILD_ID, TEST_PROJECT_KEY);
    });
  });
});
