...
import { IPipeline } from "@blocks-devops/pages/repo-details";
import NotificationListener from "./notification-listener";
import { formatFullDate } from "@/lib/utils";
import SASTLogo from "@blocks-devops/assets/icons/SAST.svg";
import SCALogo from "@blocks-devops/assets/icons/SCA.svg";
import DASTLogo from "@blocks-devops/assets/icons/DAST.png";
...
const DeploymentObservability = ({
  ...
  const displayedBuilds = showAllHistory ? sortedBuilds : sortedBuilds.slice(0, 3);

  const renderActionIcon = (action: string) => {
    switch (action) {
      case "SAST":
        return <img src={SASTLogo} width={16} height={16} alt="SAST logo" className="mr-2 h-4 w-4" />;
      case "SCA":
        return <img src={SCALogo} width={16} height={16} alt="SCA logo" className="mr-2 h-4 w-4" />;
      case "DAST":
        return (
          <img src={DASTLogo} width={12} height={12} alt="DAST logo" className="mr-2 h-3 w-3" />
        );
      default:
        return null;
    }
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-high-emphasis">
                    <div className="flex flex-wrap items-center gap-2 pb-2">
                      <span className="text-sm font-medium">{build.eventName}</span>
                ...
                <div className="flex flex-wrap items-center gap-2 pt-1">
                  {actions.map((action) => (
                    <button
                      ...
                      onClick={(event) =>
                        handleActionClick(action, build.repoId, build.itemId, event)
                      }
                    >
              ...
              <div className="flex items-center justify-between gap-2 sm:gap-4">
                <div className="text-left sm:text-right">
                  ...
                  <div className="text-xs text-gray-600">
                    Deployed in {getTimeDifference(build.createdDate, build.lastUpdatedDate)}
                  </div>
                </div>
                <Button
                  variant="ghost"
                ...
                <span className="text-sm font-medium text-gray-700">
                  <div className="flex flex-wrap items-center gap-2 pb-2">
                    ...
                    {latestBuild?.itemId === build.itemId ? (
                      ...
                      <Badge className={getDeploymentLogEventBadgeClassName(build.status)}>
                        {build.status}
                      </Badge>
                    )}
                  </div>
