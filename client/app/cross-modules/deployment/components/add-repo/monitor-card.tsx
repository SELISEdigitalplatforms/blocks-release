...
const MonitorCard = ({ repoName, url, request, emails, monitorSourceType, onOpenChange }: IMonitorCard) => {
  return (
    <Card className="rounded-lg border shadow-sm">
      ...
      <CardContent className="grid grid-cols-1 gap-x-8 gap-y-6 text-sm text-muted-foreground sm:grid-cols-2">
        ...
        {request && repoName && (
          <div>
            <div className="mb-1 text-xs font-medium text-foreground">Deploys from</div>
        ...
        {/* URL to monitor */}
        <div>
          <div className="mb-1 text-xs font-medium text-foreground">URL to monitor</div>
          <a
            href={url}
        ...
        <div className={`sm:${request ? "col-span-2" : "col-span-1"}`}>
          ...
          <div className="text-foreground">{request ? "Request" : "Callback"}</div>
        </div>
