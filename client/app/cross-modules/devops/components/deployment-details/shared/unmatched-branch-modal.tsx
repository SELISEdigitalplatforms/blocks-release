    ...
    try {
      ...
      if (result.error) {
        ...
        toast({
          ...
          description: "Unable to check branch compatibility. Please try again later.",
          variant: "destructive",
        });
        return;
      }
    ...
    switch (modalState) {
      case "loading":
        return {
          ...
          footer: (
            <Button onClick={handleClose} variant="outline" className="px-4 py-2 text-sm">
              Cancel
            </Button>
          ),
        };
        ...
        return {
          ...
          content: (
            <p className="text-sm leading-relaxed text-gray-600">
              This environment expects a branch named &apos;{repo.branch}&apos;. Please create or
              adjust the branch name in your Git repository to proceed to deployment.
            </p>
          ),
          footer: (
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative mx-4 w-full max-w-md rounded-lg bg-background shadow-xl dark:border dark:border-border">
        ...
        <div className="flex justify-end space-x-2 px-6 py-4">{modalContent.footer}</div>
      </div>
    </div>
  );
}
