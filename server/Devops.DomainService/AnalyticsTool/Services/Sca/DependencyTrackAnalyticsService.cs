...
public class DependencyTrackAnalyticsService
{
    ...
    public DependencyTrackAnalyticsService(ILogger<DependencyTrackAnalyticsService> logger, IHttpHelperServices httpHelperServices, IBuildRepository buildRepository, IRepoRepository repoRepository, DependencyTrackRepositoryService dependencyTrackRepositoryService, ICloudBuildSecret cloudBuildSecret)
    {
        _logger = logger;
    ...
    public async Task<TestReport> getInfo(string repoName, string version)
    {
        ...
        try
        {
            var url = $"{CloudBuildConstants.SCA_TOOLS_API_BASE_URI}/api/v1/project/lookup?name={formattedRepoName}&version={version}";
            ...
            var (apiCallResult, response) = await _httpHelperServices.MakeHttpGetRequest<ScaLookupResponse>(url, null, headers);
            ...
            foreach (var project in filtered)
            {
                var vulnerabilitiesUrl =
                    $"{CloudBuildConstants.SCA_TOOLS_API_BASE_URI}/api/v1/finding/project/{project.uuid}?suppressed=false";
                var (scaVulnerabilitiesResponse, vulResponse) = await _httpHelperServices.MakeHttpGetRequest<SCAVulnerability[]>(vulnerabilitiesUrl, null, headers);
        ...
        {
            _logger.LogInformation($"Failed to fetch sca data: {ex.Message}");
        }
    ...
    public async Task<string> RetrieveScaProjectUuid(Build build)
    {
        ...
        try
        {
            var url = $"{CloudBuildConstants.SCA_TOOLS_API_BASE_URI}/api/v1/project/lookup?name={formattedRepoName}&version={build.Branch}";
            ...
            var (apiCallResult, response) = await _httpHelperServices.MakeHttpRequest<ScaLookupResponse>(CloudBuildConstants.SCA_TOOLS_API_BASE_URI, url, HttpMethod.Get, null, headers);
            if (response.IsSuccessStatusCode)
            {
                _logger.LogInformation($"Successfully fetched sca data for build {build.RepoName}");
                ...
                _logger.LogInformation($"Dependencytrack Prjoect Uuid Repo update result: {repoUpdateResult}, Build update result: {buildUpdateResult}");
            }
            if (response.StatusCode == HttpStatusCode.Unauthorized)
            {
                _logger.LogError($"Authorization error for build {build.RepoName}");
            }
            if (response.StatusCode == HttpStatusCode.NotFound)
            {
                _logger.LogInformation($"No sca project found for build {build.RepoName}");
                return null;
            }
        ...
        {
            Console.WriteLine($"Failed to fetch sca data: {ex.Message}");
            return null;
        }
    }
    ...
    public async Task<bool> EnsureRepoProjectEntry(Build build, ScaLookupResponse apiCallResult)
    {
        ...
        {
            _logger.LogWarning($"No DependencyTrackProjects found for projectId {build.ProjectId}");
        ...
        if (!exists)
        {
            ...
            _logger.LogInformation($"Added RepoProject entry for RepoId {build.RepoId} with ProjectUuid {apiCallResult.uuid} to DependencyTrackProjects {dependencyTrackProject.ProjectId}");
            return true;
        }
        else
        {
            _logger.LogInformation($"RepoProject entry for RepoId {build.RepoId} already exists in DependencyTrackProjects {dependencyTrackProject.ProjectId}");
            return false;
        }
    }
}