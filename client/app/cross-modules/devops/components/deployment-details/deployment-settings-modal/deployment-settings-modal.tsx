...
import type { DeploymentFormData, IHttpError } from "@blocks-devops/models/github-info";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import { DEPLOYMENT_OPTIONS } from "@blocks-devops/models/deployment-settings";
...
const DeploymentSettingsModal = ({
  ...
  */
  ...
  useEffect(() => {
    if (isOpen && repoDetails?.data?.repo && !isInitialized && !isSpecsLoading) {
      const repo = repoDetails.data.repo;
      const preselectedDeploySettings = repo.deploySettings;
      const deploymentType = repo.deploymentType === "Manual" ? "manual" : "auto";

      // Fixed defaults: Azure, West Europe, First Active Spec
      const providers = Array.isArray(specsData) ? specsData : [];
      const azureProvider = providers.find((p: any) => p.name.toLowerCase() === "azure");
      const westEuropeRegion = azureProvider?.region?.find(
        (r: any) =>
          r.name.toLowerCase().includes("west") && r.name.toLowerCase().includes("europe"),
      );
      ...
      setDeploymentData({
        deploymentType,
        framework: "",
        provider: azureProvider?.name || preselectedDeploySettings?.hostingProvider?.name || "",
        region: westEuropeRegion?.name || preselectedDeploySettings?.region?.name || "",
        selectedSpec: firstActiveSpec?.id || preselectedDeploySettings?.machineConfig?.id || "",
        providerId: azureProvider?.id || preselectedDeploySettings?.hostingProvider?.id || "",
        regionId: westEuropeRegion?.id || preselectedDeploySettings?.region?.id || "",
        machineConfigId: firstActiveSpec?.id || preselectedDeploySettings?.machineConfig?.id || "",
      });
  ...
  */
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent
        className={
          isMobile ? "h-screen w-screen overflow-y-auto" : "max-h-[90vh] max-w-2xl overflow-y-auto"
        }
      >
        <DialogHeader>
        ...
        <div className="space-y-6 py-6">
          <div className="w-full">
            <label className="mb-3 block text-sm font-medium">Deployment Type</label>
            <RadioGroup
        ...
        <DialogFooter className="flex gap-2">
          <Button variant="outline" onClick={handleClose} disabled={isButtonDisabled()}>
            Cancel
          </Button>
