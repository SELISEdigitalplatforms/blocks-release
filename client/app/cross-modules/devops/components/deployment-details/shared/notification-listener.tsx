...
export const useDeploymentStatus = (latestBuild?: any, deploymentStatus: string = "NoBuild") => {
  ...
  const handleNotification = useCallback(
    (notificationData: any) => {
      try {
        ...
        const denormalizedPayload = JSON.parse(notificationData.message.denormalizedPayload);
...
export const DeploymentStatusBadge = ({ status }: { status: string }) => {
  return <Badge className={getDeploymentLogEventBadgeClassName(status)}>{status}</Badge>;
};
