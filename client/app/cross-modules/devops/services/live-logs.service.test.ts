...
describe("LiveLogsService", () => {
  ...
  describe("getStepStatus", () => {
    it("should map EventStarted to running", () => {
      expect(LiveLogsService.getStepStatus(DeploymentEventType.EventStarted)).toBe("running");
    });

    it("should map EventFinished to success", () => {
      expect(LiveLogsService.getStepStatus(DeploymentEventType.EventFinished)).toBe("success");
    });

    it("should map EventFailed to error", () => {
      expect(LiveLogsService.getStepStatus(DeploymentEventType.EventFailed)).toBe("error");
    });

    it("should return pending for other types", () => {
      expect(LiveLogsService.getStepStatus(DeploymentEventType.Log)).toBe("pending");
    });
  });
  ...
  describe("processLogMessage", () => {
    it("should split multiline message and filter empty lines", () => {
      ...
      expect(LiveLogsService.processLogMessage(message)).toEqual(["line 1", "line 2  ", "  line 3"]);
    });
  ...
  describe("processHistoricalLogs", () => {
    ...
    it("should calculate duration in the second loop if missing", () => {
      const mixedLogs = [
        { BuildId: MOCK_BUILD_ID, EventGroup: DeploymentEventGroup.Build, EventType: DeploymentEventType.EventStarted, CreatedAt: "2023-01-01T00:00:00Z" },
        { BuildId: MOCK_BUILD_ID, EventGroup: DeploymentEventGroup.Build, EventType: DeploymentEventType.Log, Message: "Log 1", CreatedAt: "2023-01-01T00:00:01Z" },
        { BuildId: MOCK_BUILD_ID, EventGroup: DeploymentEventGroup.Build, EventType: DeploymentEventType.EventFinished, CreatedAt: "2023-01-01T00:00:02Z" },
        { BuildId: MOCK_BUILD_ID, EventGroup: DeploymentEventGroup.Deploy, EventType: DeploymentEventType.EventFailed, CreatedAt: "2023-01-01T00:00:03Z" },
      ];
  ...
  describe("updateStepWithNotification", () => {
    it("should add new step if it doesn't exist", () => {
      ...
      const result = LiveLogsService.updateStepWithNotification(prevSteps, message);
      expect(result).toHaveLength(1);
