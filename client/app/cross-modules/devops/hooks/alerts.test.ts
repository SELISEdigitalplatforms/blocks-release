...
vi.mock("@blocks-devops/services/alerts.service", () => mockAlertsServiceFactory());
...
describe("Alert Hooks", () => {
    ...
    describe("useGetMonitorList", () => {
        it("should fetch monitor list successfully", async () => {
            vi.mocked(alertsService.getMonitorListById).mockResolvedValue(mockMonitorList);

            const { result } = renderHook(() => useGetMonitorList(TEST_PROJECT_KEY, MOCK_REPO_ID), {
                wrapper: createWrapper(),
            });

            await waitFor(() => expect(result.current.isSuccess).toBe(true));
            expect(result.current.data).toEqual(mockMonitorList);
            expect(alertsService.getMonitorListById).toHaveBeenCalledWith(TEST_PROJECT_KEY, MOCK_REPO_ID);
        });
    });
    ...
    describe("useDeleteMonitor", () => {
        it("should call deleteSingleMonitor on success", async () => {
            vi.mocked(alertsService.deleteSingleMonitor).mockResolvedValue({ isSuccess: true } as any);
            ...
            await waitFor(() => expect(result.current.isSuccess).toBe(true));
            expect(alertsService.deleteSingleMonitor).toHaveBeenCalledWith(MOCK_MONITOR_ID);
        });
    });
    ...
    describe("useGetMonitorDetails", () => {
        it("should fetch monitor details successfully", async () => {
            ...
            vi.mocked(alertsService.getMonitorDetails).mockResolvedValue(mockDetails as any);

            const { result } = renderHook(() => useGetMonitorDetails(MOCK_MONITOR_ID), {
                wrapper: createWrapper(),
            });
    ...
    describe("useGetMonitorById", () => {
        it("should fetch monitor by id successfully", async () => {
            vi.mocked(alertsService.getMonitorById).mockResolvedValue({ data: mockAlert, isSuccess: true } as any);
    ...
    describe("useGetMonitorResponseTime", () => {
        it("should fetch monitor response time successfully", async () => {
            const option = { monitorId: MOCK_MONITOR_ID, timeRange: "1h", interval: 60 };
            vi.mocked(alertsService.GetMonitorResponseTime).mockResolvedValue({ data: [], isSuccess: true } as any);
            ...
            await waitFor(() => expect(result.current.isSuccess).toBe(true));
            expect(alertsService.GetMonitorResponseTime).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe("useSaveHealth", () => {
        it("should call saveHealth on success", async () => {
            ...
            vi.mocked(alertsService.saveHealth).mockResolvedValue({ isSuccess: true, data: { itemId: MOCK_MONITOR_ID } } as any);
    ...
    describe("useUpdateSingleMonitor", () => {
        it("should call updateSingleMonitor on success", async () => {
            ...
            vi.mocked(alertsService.updateSingleMonitor).mockResolvedValue({ isSuccess: true, data: { itemId: MOCK_MONITOR_ID } } as any);
    ...
    describe("useGetHealthMonitorList", () => {
        it("should fetch health monitor list successfully", async () => {
            vi.mocked(alertsService.getHealthMonitorList).mockResolvedValue(mockMonitorList);

            const { result } = renderHook(() => useGetHealthMonitorList(TEST_PROJECT_KEY, 1, 1, 10), {
                wrapper: createWrapper(),
            });

            await waitFor(() => expect(result.current.isSuccess).toBe(true));
            expect(result.current.data).toEqual(mockMonitorList);
            expect(alertsService.getHealthMonitorList).toHaveBeenCalledWith(TEST_PROJECT_KEY, 1, 1, 10);
        });
    });
    ...
    describe("useGetMonitorListById", () => {
        it("should fetch monitor list by id successfully", async () => {
            ...
            const { result } = renderHook(() => useGetMonitorListById(TEST_PROJECT_KEY, MOCK_REPO_ID), {
                wrapper: createWrapper(),
            });
    ...
    describe("useGetAllIncidentList", () => {
        it("should fetch all incident list successfully", async () => {
            ...
            vi.mocked(alertsService.getAllMonitorIncidentList).mockResolvedValue(mockResponse as any);

            const { result } = renderHook(() => useGetAllIncidentList(MOCK_MONITOR_ID, 1, 10), {
                wrapper: createWrapper(),
            });

            await waitFor(() => expect(result.current.isSuccess).toBe(true));
            expect(result.current.data).toEqual(mockResponse);
            expect(alertsService.getAllMonitorIncidentList).toHaveBeenCalledWith(MOCK_MONITOR_ID, 1, 10);
        });
    });
    ...
    describe("useGetMonitorDownTime", () => {
        it("should fetch monitor down time successfully", async () => {
            const option = { monitorId: MOCK_MONITOR_ID, timeRange: "1h", interval: 60 };
            vi.mocked(alertsService.GetMonitorDownTime).mockResolvedValue({ data: [], isSuccess: true } as any);
            ...
            await waitFor(() => expect(result.current.isSuccess).toBe(true));
            expect(alertsService.GetMonitorDownTime).toHaveBeenCalledWith(expect.objectContaining({
                monitorId: MOCK_MONITOR_ID,
            }));
        });
    });
    ...
    describe("useUpdateHealth", () => {
        it("should call updateHealth on success", async () => {
            const payload = { itemId: MOCK_MONITOR_ID, isActive: false };
            vi.mocked(alertsService.updateHealth).mockResolvedValue({ isSuccess: true, data: { tenantId: TEST_PROJECT_KEY } } as any);
    ...
    describe("useDeleteHealth", () => {
        it("should call deleteHealth on success", async () => {
            vi.mocked(alertsService.deleteHealth).mockResolvedValue({ isSuccess: true } as any);
