...
same shape as `blocks-logic/e2e`, `blocks-data/e2e`, and `blocks-utilities/e2e`.
...
1. **Suite setup** (`tests/suite/suite.setup.spec.ts`) — OIDC login, reuse or create one shared project, write `release-project.json`, then save `release-session.json` **after** the dashboard is open (so localStorage keeps project/env). Does **not** open Deployment.
2. **Features** (`tests/01-overview`, `tests/02-deployment`, …) — use session; open shared dashboard with a direct `goto` to `/app/{itemId}/dashboard`, then feature nav via helpers.
...
e2e/
  ...
  support/
    ...
    run-outcome.ts                # markReleaseTestFailed
