...
import { useGithubBranches } from "@blocks-devops/hooks/github-info";
import WarningBanner from "../import-repo/helpers/warning-banner";
import type { IBranch, IRepository } from "@blocks-devops/models/github-info";
import {
  DropdownMenu,
  DropdownMenuTrigger,
  DropdownMenuContent,
...
const RepositorySelector = ({
  ...
  const defaultBranches: IBranch[] = [{ name: "main", commit: { sha: "", url: "" } }];

  const apiBranches: IBranch[] =
    selectedRepo && branchesData ? (Array.isArray(branchesData) ? branchesData : []) : [];
  ...
  const filteredBranches = validBranches.filter(
    (branch) => branch.name && branch.name.toLowerCase().includes(branchSearchTerm.toLowerCase()),
  );
  ...
  return (
    <div className="flex h-auto w-full flex-col items-start justify-center gap-5 self-stretch rounded-sm border border-border-default bg-background px-5 pb-6 pt-5">
      <h3 className="text-lg font-semibold">Select your repository and branch</h3>
      <WarningBanner />
      {repositories ? (
                ...
                >
                  <div className="flex items-center gap-2 truncate">
                    {selectedRepo ? (
                      ...
                      <span className="text-low-emphasis">Select repository</span>
                    )}
                  </div>
                ...
                <div className="max-h-[200px] overflow-y-auto">
                  ...
                  {filteredRepositories.length > 0 ? (
                    filteredRepositories.map((repo: IRepository) => (
                      <DropdownMenuItem
                        key={repo.id}
                        onClick={() => handleSelectRepo(repo.full_name, repo.html_url)}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-md border border-blocks-primary-shades-300 px-3 py-2 text-left focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary ${!selectedRepo ? "cursor-not-allowed opacity-50" : "bg-background"
                    }`}
                >
                  <div className="flex items-center gap-2 truncate">
                    {selectedBranch ? (
        ...
        <div className="w-full py-4 text-center text-low-emphasis">No repositories available</div>
      )}
    </div>
  );
};
