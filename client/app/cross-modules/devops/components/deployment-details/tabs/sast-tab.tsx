...
import { useGetSASTData, useSASTRedirectLink } from "@blocks-devops/hooks/observability";
...
interface OverviewStatsProps {
  stats: StatItem[];
  gradeStatusView: (grade: string, title: string, value: string) => React.ReactNode;
  title?: string;
  description?: string;
  gridColumns?: string;
  className?: string;
...
const OverviewStats = ({
  ...
  return (
    <div className={`w-full space-y-4 ${className}`}>
      ...
      {description && <p className="text-sm text-medium-emphasis">{description}</p>}
  ...
  return [
    {
      ...
      grade: details?.software_quality_security_rating === "1.0" ? "A" : "alert",
    },
    {
      title: "Reliability",
      value: details?.software_quality_reliability_issues ?? "0",
      subtitle: "Open issues",
      grade: details?.software_quality_reliability_rating === "1.0" ? "A" : "alert",
    },
    {
    ...
    {
      title: "Duplications",
      value: details?.duplicated_lines_density ? `${details.duplicated_lines_density}%` : "0%",
...
const gradeStatusView = (grade: string, title: string, value: string) => {
  const commonStyle = "flex h-12 w-12 items-center justify-center rounded-full text-lg font-bold";
  ...
  if (!isNaN(numericValue)) {
    if (numericValue < 5) {
      return (
        <div className="flex items-center gap-2">
          <span className={`${commonStyle} bg-green-100 text-green-700`}>A</span>
        </div>
      );
...
const SastTab = () => {
  ...
  const { isLoading: isSASTLoading, refetch: triggerSASTRedirect } = useSASTRedirectLink(buildId);
      ...
      {isLoading ? (
        ...
        <Card className="w-full">
          <CardHeader className="pb-4 text-center">
            ...
            <CardTitle className="text-2xl">Static Application Security Testing</CardTitle>
        ...
        <Card className="w-full">
          <CardHeader className="pb-4 text-center">
            ...
            <CardTitle className="text-2xl">Static Application Security Testing</CardTitle>
            <CardDescription className="text-lg">Error Loading Data</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4 text-center">
            <p className="text-muted-foreground">
