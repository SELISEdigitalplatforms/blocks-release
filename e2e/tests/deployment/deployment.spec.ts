...
test.describe("Deployment with GitHub repo during project creation", () => {
  ...
  test("should create a project with GitHub repo", async ({ page }) => {
    ...
    // Note: GitHub auth is already handled by the setup project, so we skip the popup part
    await osCreateProjectPage.selectRepository(
  ...
  test("should deploy the project", async ({ page }) => {
    ...
    await deploymentPage.expectDeploymentStartedToast();
    await deploymentPage.expectSuccessfullyDeployedToast();
  });

  test("should check deployment details", async ({ page }) => {
    await deploymentPage.clickSASTButton();
    await deploymentPage.expectSASTPassed();
    await deploymentPage.clickSCAButton();
    await deploymentPage.clickDASTButton();
    await deploymentPage.clickDeploymentLogsButton();
  });
